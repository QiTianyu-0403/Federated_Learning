...
def get_dataset_cifar10_noniid(args):
    ...
    if args.noniid_model == 'label_noniid':
        data_users_train = cifar_extr_label_noniid(train_dataset, args)
    if args.noniid_model == 'quantity_noniid':
        data_users_train = cifar_extr_quantity_noniid(train_dataset, args)
...
def cifar_extr_label_noniid(train_dataset, args):
    ...
    # shuffle
    for i in range(10):
        np.random.shuffle(idxs_labels[0][5000 * i:5000 * (i+1)])
    idxs = idxs_labels[0, :]

    # equal part
    for i in range(args.num_users):
        for j in range(10):
            dict_users_train[i] = np.concatenate((dict_users_train[i], idxs[j * 5000 + i * num_per_equal: j * 5000 + (1 + i) * num_per_equal]), axis=0)

    # noniid init part
    residue_noniid = 50000 - num_per_equal * 10 * args.num_users
    ...
    for i in range(10):
        delete_rank = np.append(delete_rank, np.arange(i * 5000,  args.num_users * num_per_equal + i * 5000, 1))
    for i in range(len(surplus_list)):
        delete_rank = np.append(delete_rank, np.arange((i + 1) * 5000 - surplus_list[i], (i + 1) * 5000, 1))
...
def cifar_extr_quantity_noniid(train_dataset, args):
    ...
    for i in range(args.num_users):
        for j in range(10):
            dict_users_train[i] = np.concatenate((dict_users_train[i], idxs[j * 5000 + i * num_per_user_list[i]: j * 5000 + (1 + i) * num_per_user_list[i]]), axis=0)
...
def cifar_extr_iid(train_dataset, args):
    dict_users_train = {i: np.array([]) for i in range(args.num_users)}
    idxs = np.arange(50000)
    labels = np.array(train_dataset.targets)
    ...
    for i in range(args.num_users):
        for j in range(10):
            dict_users_train[i] = np.concatenate((dict_users_train[i], idxs[j * 5000 + i * num_per_user: j * 5000 + (1 + i) * num_per_user]), axis=0)

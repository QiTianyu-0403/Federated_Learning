...
class Server(object):
    ...
    def run_episode(self, i):
        ...
        for worker_rref in self.worker_rrefs:
            futs.append(rpc.rpc_async(worker_rref.owner(), _call_method, args=(Worker.run_episode, worker_rref, para),
                                      timeout=0))
        for fut in futs:
            update_paras.append(fut.wait())
        self.model_average(update_paras)
        accuracy = self.evaluate()
        print("Episode {}:server valid accuracy is {}%".format(i, accuracy))
            ...
            for epoch in range(pre_epoch, args.epoch_worker):
                ...
                for i, data in enumerate(self.train_loader, 0):
                    ...
                    print('[epoch:%d, iter:%d] Loss: %.03f | Acc: %.3f%% ----Rank%d'
                          % (epoch + 1, (i + 1 + epoch * length), sum_loss / (i + 1), 100. * correct / total, args.rank))
